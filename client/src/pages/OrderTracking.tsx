...
const OrderTracking = () => {
  ...
  useEffect(()=>{
    api.get(`/orders/${id}`).then((res)=> setOrder(res.data.order)).catch(() =>navigate("/orders"))
  ...
  useEffect(()=>{
    ...
    const fetchLocation = async () => {
      try {
        ...
        // Update order status if is changed
  ...
  return (
    <div className="min-h-screen mb-20 bg-app-cream">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Delivery Person */}
        {order?.deliveryPartner && order.status !== "Delivered" && order.
        status !== "Cancelled" && (
          ...
          <div className="bg-white rounded-2xl p-5">
            ...
            <div className="mt-4 pt-3 text-sm border-t border-app-border space-y-1.5">
              ...
              <div className="flex justify-between">
                ...
                <span className="">{order?.deliveryFee === 0 ? "Free" : 
                  `${currency}${order?.tax.toFixed(2)}`}</span>
              </div>
