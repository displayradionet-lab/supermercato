...
export default function AdminOrders() {
  ...
  const fetchPartners = async () => {
    try {
      ...
      // Filtro corretto per prendere solo i partner attivi
  ...
  // 🟢 FIX: Caricamento centralizzato nel useEffect
  useEffect(() => {
    const initDashboard = async () => {
      setLoading(true);
      // Esegue i fetch in parallelo per una UX super veloce, aspettando che entrambi finiscano
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      ...
      await fetchOrders(); // Ricarica la lista aggiornata
    } catch (error: any) {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      setAssignModal(null);
      setSelectedPartner('');
      await fetchOrders(); // Ricarica la lista dopo l'assegnazione
    } catch (error: any) {
      toast.error(
