...
export default function AdminProductForm() {
 ...
 const fetchData = async () => {
    ...
    try {
      ...
      // 🟢 Facciamo la chiamata reale al backend usando l'ID del database
      const { data } = await api.get(`/products/${id}`); 
      
      // Controlla la struttura della risposta del tuo backend (es. data.product o data)
      ...
      console.error("Errore durante il caricamento del prodotto dal DB:", error);
      toast.error(error.response?.data?.message || "Impossibile caricare il prodotto");
    } finally {
      setLoading(false);
    }
  };
    ...
    const handleSubmit = async (e: React.SubmitEvent) => {
        ...
        try {
            ...
            if (imageFile){
                ...
                finalImageUrl = data.url || data.imageUrk || data.path;
            } 
            if (!finalImageUrl){
